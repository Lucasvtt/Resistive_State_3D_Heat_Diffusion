program resistive_state_xi
	...
	character(len=20) pxyw,pxzw,pyzw,phxyw,phxzw,phyzw,hxw,hyw,hzw,vw,vtcyw,vtczw,jxw,jxbw,jyw,jzw,htyw,htzw,jnxw,jnyw,txyw,tyzw,txw,potw
					...
					do j = 2,Ny
						do i = 2,Nx
							delP(i,j,k) = +rx*(Ux(i,j,k)*G(i+1,j,k)-2.0d0*G(i,j,k)+dconjg(Ux(i-1,j,k))*G(i-1,j,k))  &
										  ...
										  +c*G(i,j,k)*(1-T0-cdabs(G(i,j,k))**2)
                        end do
					end do
				end do
			...
			do k  = 1,Nz+1
				do j = 1,Ny
					do i = 1,Nx
						...
						G(i,j,k) = G(i,j,k)+1.0d-2*(2.0d0*px-1.0d0)
					end do
				end do
			end do
		end if
